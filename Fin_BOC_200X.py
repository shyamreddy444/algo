...
def GetStrikeList(df_oc, option_lower_range, option_upper_range):
    Message = "I am inside GetStrikeList, to find out strike in range of "+ str(option_lower_range) + "-" + str(option_upper_range)
    print(Message)
    logger.info(Message)
    strikeList = []
    try:
        ...
        Message = str(e) + ": Exception occur in GetStrikeList"
        print(Message)
        logger.info(Message)
    return strikeList
...
def strategy():
    ...
    snapshot_hour, snapshot_minute, snapshot_second = OptionSnapShotTime.split(':')

    if dt.now(timezone("Asia/Kolkata")).time() > time(int(snapshot_hour),int(snapshot_minute),int(snapshot_second)):
    ...
    while dt.now(timezone("Asia/Kolkata")).time() < time(int(snapshot_hour),int(snapshot_minute),int(snapshot_second)):
        Message = "Waiting for option snapshot time " + OptionSnapShotTime 
        logger.info(Message)
        print(Message)
        sleep(5)
    ...
    strikeList = GetStrikeList(df_oc, option_lower_range, option_upper_range)
    
    while len(strikeList) == 0:
        Message = "No option found in provided range " + str(option_lower_range) + " - " + str(option_upper_range) + ", Do you want to reduce lower range , enter Yes/No"
        print(Message)
        logger.info(Message)
        ...
        if (Userchoice[0] == 'Y'):
            Message = "Please enter updated lower range less than : " + str(option_lower_range)
            logger.info(Message)
            print(Message)
            
            Updated_option_lower_range = input("Enter new lower range : ")
            if Updated_option_lower_range.isnumeric():
                option_lower_range = float(Updated_option_lower_range)
                #print(Userchoice)
                logger.info(Updated_option_lower_range)
                strikeList = GetStrikeList(df_oc, option_lower_range, option_upper_range)
            else:
                Message = "Incorrect input provided by you, kindly enter integer value e.g. 150,160,.."
                print(Message)
                logger.info(Message)
        else:
    ...
    df_trade_option['Target'] = None
    df_trade_option['SL'] = None
    ...
    EntryTime_hour, EntryTime_minute, EntryTime_second = EntryTime.split(':')

    while dt.now(timezone("Asia/Kolkata")).time() < time(int(EntryTime_hour),int(EntryTime_minute),int(EntryTime_second)):
        Message = "Waiting for trade entry time " + EntryTime 
        logger.info(Message)
        print(Message)
        sleep(5)
                ...
                for ind in df_trade_option.index:
                    ...
                    if(df_trade_option['TradeStatus'][ind] == 'Wait'):
                        if Ltp > BuyAbove:
                            ...
                            df_trade_option['Target'][ind] = Ltp + int(Target_Point)
                            df_trade_option['SL'][ind] = Ltp - int(SL_Point)
                            df_trade_option['EntryPrice'][ind] = Ltp
                            df_trade_option['TradeStatus'][ind] = 'Active'
                    elif(df_trade_option['TradeStatus'][ind] == 'Active'):
                        if Ltp > df_trade_option['Target'][ind] or Ltp < df_trade_option['SL'][ind] :
...
if __name__ == '__main__':  
    ...
    WelcomeMessage = "\n\nBOC 200X algo started for " + userid
